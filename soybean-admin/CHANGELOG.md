...
- Renamed the project to AmiDesk API Server across the Go module, CLI binary, Docker image, container names and administration panel.
- Updated the panel title, footer attribution and repository references to the AmiDesk project.

## v2.0 (2026-08-28)

### Amitronic fork

- Added anonymous HTTPS delivery of the RustDesk hbbs public key and service endpoints through the stock client heartbeat contract.
- Added administrator TOTP enforcement controls and account-based login lockout after five failed attempts.
- Set Europe/Warsaw as the deployment timezone and English (UK) as the default administration language.
- Added Docker build reproducibility, persistent first-start configuration, and Linux-compatible container startup scripts.
- Removed upstream sponsorship content and Chinese default UI content from the administration panel.

### Previous release

- Based on the existing AmiDesk API Server and SoybeanAdmin `v1.3.4` codebase, including the earlier `v1.2.7` RustDesk client compatibility update.
