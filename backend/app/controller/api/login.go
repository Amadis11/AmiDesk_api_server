...
import (
	...
	"amidesk-api-server/app/service"
...
func (c *LoginController) PostLogin() mvc.Result {
	...
	identity := loginForm.Username
	if identity == "" {
		identity = loginForm.RustdeskId
	}
	if !security.Allow(c.Cfg.Security, "api", identity) {
		return mvc.Response{Object: iris.Map{"error": "TooManyLoginAttempts"}}
	}
	...
	if loginForm.Type == "email_code" && loginForm.VerificationCode != "" && loginForm.TfaCode == "" { // email_code 开始验证
		...
		return c.loginResult(result, identity)
	}
...
func (c *LoginController) loginResult(result iris.Map, identity string) mvc.Result {
	if _, authenticated := result["access_token"]; authenticated {
		security.Clear("api", identity)
	} else if _, challenge := result["tfa_type"]; !challenge {
		security.RecordFailure(c.Cfg.Security, "api", identity)
	}
