...
AmiDesk API Server is an open source API server for the open source [RustDesk](https://github.com/rustdesk/rustdesk) client. It implements the client API interfaces and provides a Web-UI for data management.
...
- Synchronized RustDesk version (currently adapted client: 1.4.6)
...
db:
  driver: "sqlite"
  dsn: "./server.db"
  timeZone: "Europe/Warsaw" # setting the time zone fixes the database creation time problem
  showSql: false
  # driver: "mysql"
...
```shell
docker exec amidesk-api-server amidesk-api-server user add admin yourpassword --admin
```
...
```shell
git clone https://github.com/Amadis11/AmiDesk_api_server.git
```
...
Assuming the compiled binary file is called `amidesk-api-server.exe`.

1. Synchronize the database table structure

```shell
amidesk-api-server.exe sync
```

2. Add your first user

```shell
amidesk-api-server.exe user add admin yourpassword --admin
```
...
Here's a backend reverse proxy configuration for you to refer to:
...
Flags:
  -h, --help   help for amidesk-api-server

Use "amidesk-api-server [command] --help" for more information about a command.
```
...
> You can view the full license [here](https://github.com/Amadis11/AmiDesk_api_server/blob/main/LICENSE)

This project is under the terms of the **AGPL-3.0** license.
